    ...
    <div class="card shadow rounded border-0">
        ...
        <div class="card-body">
            <dl class="row mb-4">
                ...
                <dd class="col-sm-8 fs-5">{{ $leaveRequest->leaveType->name ?? '-' }}</dd>

                <dt class="col-sm-4 text-muted">Duration (days)</dt>
                <dd class="col-sm-8 fs-5">{{ $leaveRequest->duration }}</dd>

                <dt class="col-sm-4 text-muted">Start Date & Time</dt>
                <dd class="col-sm-8 fs-5">
                    {{ \Carbon\Carbon::parse($leaveRequest->start_date)->format('d M Y') }}
                    <span class="badge bg-info text-dark ms-2 text-capitalize">{{ $leaveRequest->start_time }}</span>
                </dd>

                <dt class="col-sm-4 text-muted">End Date & Time</dt>
                <dd class="col-sm-8 fs-5">
                    {{ \Carbon\Carbon::parse($leaveRequest->end_date)->format('d M Y') }}
                    <span class="badge bg-info text-dark ms-2 text-capitalize">{{ $leaveRequest->end_time }}</span>
                </dd>

                <dt class="col-sm-4 text-muted">Reason</dt>
                <dd class="col-sm-8 fs-5"><pre class="mb-0" style="white-space: pre-wrap;">{{ $leaveRequest->reason ?? '-' }}</pre></dd>
